...
private Map<String, String> checksumArtifacts(String algorithm, MavenProject project) {
  ...
  // POM
  checksums.put(project.getArtifactId() + ".pom", checksum(algorithm, project.getFile()));

  // Main artifact
  Artifact artifact = project.getArtifact();
  if (artifact != null && artifact.getFile() != null) {
    checksums.put(artifact.getFile().getName(), checksum(algorithm, artifact.getFile()));
  }

  // attached artifacts
  project.getAttachedArtifacts()
          .forEach(attachedArtifact ->
                  checksums.put(attachedArtifact.getFile().getName(), checksum(algorithm, attachedArtifact.getFile())));
